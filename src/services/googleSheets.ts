 ...
 * 1. O script busca a URL do Firebase em 'Propriedades do Script' (FIREBASE_DB_URL)
 *    ou utiliza a URL padrão do projeto SAGA WMS.
 * 2. No Firebase Console, certifique-se de que as Regras (Rules) do Realtime Database estão:
 *    {
 *      "rules": {
 *        ".read": true,
 *        ".write": true
 *      }
 *    }
 * 3. O aplicativo (frontend) escuta via 'onValue' no nó '/ranking_atual' de forma instantânea.
 */

function obterFirebaseUrl() {
  var propUrl = PropertiesService.getScriptProperties().getProperty("FIREBASE_DB_URL");
  var baseUrl = propUrl && propUrl.trim() !== "" 
    ? propUrl.trim() 
    : "https://ranking-produtividade-default-rtdb.firebaseio.com";
  
  // Limpa trailing slash
  baseUrl = baseUrl.replace(/\\/+$/, "");
  
  if (baseUrl.indexOf("/ranking_atual.json") !== -1) {
    return baseUrl;
  }
  return baseUrl + "/ranking_atual.json";
}
...
function onOpen() {
  try {
    ...
    ui.createMenu('⚡ SAGA WMS')
      ...
      .addSeparator()
      .addItem('⏰ Agendar Sincronização por Tempo (A cada hora)', 'agendarSincronizacaoAutomatica')
      .addItem('⚡ Ativar Sincronização Instantânea em Edições', 'ativarGatilhoEdicao')
      .addToUi();
  } catch (e) {
...
function sincronizarComAppSaga() {
  ...
  try {
    ...
    if (!data || data.length < 2) {
      if (ui) ui.alert('⚠️ A planilha está vazia ou possui apenas o cabeçalho.');
      return;
    }
    ...
    var targetUrl = obterFirebaseUrl();
    var options = {
      method: 'put',
    ...
    if (code === 200) {
      if (ui) ui.alert('✅ SUCESSO!\n\n' + operators.length + ' colaboradores válidos sincronizados com sucesso no Firebase Realtime Database!\n\nNó Firebase: /ranking_atual');
    } else {
      if (ui) ui.alert('⚠️ Resposta do Servidor (Código ' + code + '):\n' + response.getContentText());
    }

  } catch (err) {
    if (ui) ui.alert('❌ ERRO NO SCRIPT:\n\n' + err.toString());
  }
}
...
function agendarSincronizacaoAutomatica() {
  ...
  try {
    ScriptApp.newTrigger('sincronizarComAppSaga')
      ...
      .everyHours(1)
      .create();
    if (ui) ui.alert('⏰ Sincronização automática agendada a cada 1 hora!');
  } catch (e) {
    if (ui) ui.alert('❌ Erro ao agendar: ' + e.toString());
  }
}

function ativarGatilhoEdicao() {
  var ui = SpreadsheetApp.getUi();
  try {
    ScriptApp.newTrigger('sincronizarComAppSaga')
      ...
      .onChange()
      .create();
    if (ui) ui.alert('⚡ Gatilho ativado! Qualquer alteração ou deleção de linha na planilha sincronizará instantaneamente com o Firebase!');
  } catch (e) {
    if (ui) ui.alert('❌ Erro ao criar gatilho de edição: ' + e.toString());
  }
}
`;
